...
trait ReadJournalProvider extends LazyLogging {
  ...
  private def findReadJournalSetting(): String = {
    ...
    logger.warn("Missing conf 'qollabor.read-journal'. Trying to determine read journal settings by guessing based on the name of the journal plugin \"" + configuredJournal + "\"")
