...
import org.qollabor.cmmn.akka.command.casefile.{CreateCaseFileItem, DeleteCaseFileItem, ReplaceCaseFileItem, UpdateCaseFileItem}
...
class CaseFileRoute(val caseQueries: CaseQueries)(override implicit val userCache: IdentityProvider) extends CasesRoute {
  ...
  /**
    ...
    * @return
    */
  private def casefileContentRoute(action: String, subRoute: (PlatformUser, Value[_], String, org.qollabor.cmmn.instance.casefile.Path) => Route): Route = {
  ...
  /**
    ...
    * @return
    */
  private def casefileRoute(action: String, subRoute: (PlatformUser, String, org.qollabor.cmmn.instance.casefile.Path) => Route): Route = {
    validUser { platformUser =>
  ...
  /**
    * Checks if the path ends or has more elements left, and returns any remains into a Qollabor Path object.
    * Empty path (either with or without slash results in an empty path object, referring to top level CaseFile)
    * @param subRoute
    * @return
    */
  private def withCaseFilePath(subRoute: org.qollabor.cmmn.instance.casefile.Path => Route): Route = {
    // Creating a "qollabor-path" will validate the syntax
    import org.qollabor.cmmn.instance.casefile.Path
    pathEndOrSingleSlash {
      subRoute(new Path(""))
    } ~ path(Remaining) { rawPath =>
      // Take the "raw" remaining string, and decode it, and make it a Qollabor CaseFile Path
      // Note: taking "Segment" or "Segments" instead of "Remaining" fails and returns 405 on paths like "abc[0 ",
      //  when parsing it to a Qollabor path the error message is more clear.
