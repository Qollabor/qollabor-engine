package org.qollabor.service.api
...
import org.qollabor.infrastructure.Configured

class SwaggerHttpServiceRoute(override val apiClasses: Set[Class[_]]) extends SwaggerHttpService with Configured {

  lazy val configuredHost = config.getString("qollabor.api.bindhost")
  lazy val configuredPort = config.getString("qollabor.api.bindport")
  ...
  override val info = Info(description =
    """HTTP JSON interface to the Qollabor APIs""".stripMargin, version = "1.0.0")
